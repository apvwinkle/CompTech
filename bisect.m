function xval = (fun, aguess, bguess)
%This function solves for the root of string function 'f' with initial
...
for i = 1:1000;
    ...
    elseif f(xbisect)*f(aguess) = 0;
        val = xbisect; %We already know that a and b aren't 0, so if the product of f(xbisect) with f(aguess) will give a 0 only if f(xbisect) = 0
    end
